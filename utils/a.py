...
from config.CFG import TrackingConfig
...
class VideoProcessor:
    def __init__(
            ...
            zone_in_polygons: List[np.ndarray] = None,
            zone_out_polygons: List[np.ndarray] = None,
            output_folder: str = "pig_sequences"
    ) -> None:
        ...
        self.pig_sequences: Dict[int, List[np.ndarray]] = {}
    ...
    def annotate_frame(self, frame: np.ndarray, detections: sv.Detections) -> np.ndarray:
        ...
        for i, (zone_in_annotator, zone_out_annotator, box_in_annotator, box_out_annotator) in enumerate(
            ...
            if detections is not None:
                ...
                detections_out_zone = detections[detections.zone_id == -1]
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        if TrackingConfig.ENABLE_COUNTING:
            ...
            self.process_pig_sequences(frame, detections)
        else:
            detections = None
    ...
    def process_pig_sequences(self, frame: np.ndarray, detections: sv.Detections):
        if detections is None:
            return

        for i in range(len(detections)):
            class_id = int(detections.class_id[i])
            if self.model.names[class_id] == "pig":  # Assuming "pig" is the class name for pigs
                tracker_id = int(detections.tracker_id[i])
                bbox = detections.xyxy[i]
                pig_image = frame[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])]

                if tracker_id not in self.pig_sequences:
                    self.pig_sequences[tracker_id] = []

                self.pig_sequences[tracker_id].append(pig_image)

                if len(self.pig_sequences[tracker_id]) == TrackingConfig.SEQUENCE_LENGTH:
                    self.save_pig_sequence(tracker_id)


    def save_pig_sequence(self, tracker_id: int):
                    sequence = self.pig_sequences[tracker_id]
                    for i, img in enumerate(sequence):
                        output_path = os.path.join(self.output_folder, f"pig_{tracker_id}_{i}.jpg")
                        cv2.imwrite(output_path, img)

                    # Clear the sequence after saving to save memory
                    del self.pig_sequences[tracker_id]
